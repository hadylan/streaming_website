...
if (!isset($_SESSION['user']['id'])) {
    ...
    if ( !empty($_POST['title']) and !empty($_POST['description']) and !empty($_POST['picture']) and !empty($_POST['link']) and !empty($_POST['id_category']) and !empty($_POST['id_gender']) and !empty($_POST['year']) and !empty($_POST['id']) ) 
    {
        # --- Call abstract class Database for insert data in `movies` table with prepared query and dataMapper ---
        Database::query('UPDATE `movies` SET `title`=:title,`description`=:description,`picture`=:picture,`link`=:link,`id_category`=:id_category,`id_gender`=:id_gender,`year`=:year WHERE `id` = :id',
        array(
                # --- Call sanitize class for clean POST request datas ---
            'id' => $sanitize->sanitizeInteger(intval($_POST['id'])),
